...
fun NavGraphBuilder.quoteAddEdit(
    ...
    composable(
        route = Screen.QuoteAddEdit.name + "?itemId={itemId}",
        ...
        val quoteUiState by quoteViewModel.quoteUiState.collectAsState()
        ...
        // Product.type visibility: read from quoteUiState.documentIssuer (authoritative
        ...
        val showProductType = quoteUiState.documentIssuer?.intraEuSales == true
        ...
        DocumentAddEditPlatform(
            ...
            onSelectClientOrIssuer = { clientOrIssuer ->
                ...
                quoteViewModel.saveDocumentClientOrIssuerInLocalDb(documentClientOrIssuer)
                quoteViewModel.saveDocumentClientOrIssuerInUiState(documentClientOrIssuer)
            },
            ...
            onClickDeleteDocumentProduct = {
                quoteViewModel.removeDocumentProductFromUiState(it)
                quoteViewModel.removeDocumentProductFromLocalDb(it)
            },
            onClickDeleteDocumentClientOrIssuer = { type ->
                quoteViewModel.removeDocumentClientOrIssuerFromUiState(type)
                quoteViewModel.removeDocumentClientOrIssuerFromLocalDb(type)
            },
            placeCursorAtTheEndOfText = { pageElement ->
                if (pageElement == ScreenElement.DOCUMENT_NUMBER ||
                    ...
                    quoteViewModel.updateTextFieldCursorOfQuoteState(pageElement)
                }
            },
                    ...
                    when (typeOfCreation) {
                        ...
                        DocumentBottomSheetTypeOfForm.NEW_PRODUCT -> {
                            if (productAddEditViewModel.validateInputs(ProductType.DOCUMENT_PRODUCT)) {
                                ...
                                val documentProductId = quoteViewModel.saveDocumentProductInLocalDbAndGetId(docProductWithLink)
                                if (documentProductId != null) {
                                    quoteViewModel.saveDocumentProductInUiState(docProductWithLink.copy(id = documentProductId))
                        ...
                        DocumentBottomSheetTypeOfForm.EDIT_PRODUCT -> {
                            if (productAddEditViewModel.validateInputs(ProductType.DOCUMENT_PRODUCT)) {
                                quoteViewModel.updateUiState(ScreenElement.DOCUMENT_PRODUCT, documentProduct)
            ...
            onOrderChange = quoteViewModel::updateDocumentProductsOrderInUiStateAndDb,
