...
fun NavGraphBuilder.documentProductTaxRates(
    navController: NavController,
    onClickBackOrSelect: () -> Unit
) {
    composable(route = Screen.DocumentProductTaxRates.name) { backStackEntry ->
        ...
        ProductTaxRates(
            navController = navController,
            taxRates = taxRates,
            currentTaxRate = viewModel.productUiState.value.taxRate ?: BigDecimal(0.0),
